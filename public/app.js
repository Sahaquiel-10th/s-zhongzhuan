...
GET ${escapeHtml(base)}/usage?page=1&amp;page_size=10</pre><p class="panel-note"><code>/usage</code> 每页支持 5 或 10 条。鉴权可使用 <code>Authorization: Bearer sk-...</code> 或 <code>x-api-key: sk-...</code>。这些接口不能充值、改价或修改账户。价格更新时左下角铃铛会出现红点。</p></div></div>
      ...
      <h3 class="guide-title">几个容易混淆的词</h3><div class="guide-grid"><article><strong>电力</strong><p>账户余额和调用结算使用的统一单位。</p></article><article><strong>成交价</strong><p>实际扣费使用的输入、缓存读取和输出单价。</p></article><article><strong>官方参考价</strong><p>只用于对比展示，不直接决定本次实扣。</p></article><article><strong>综合倍率</strong><p>本次实际费用与官方参考费用的比值。</p></article></div>
    </section>`;
  },
  usage() {
    setPageMeta('USAGE', '计量日志');
    const rows = state.customer.usage;
    $('#pageContent').innerHTML = `<section class="panel"><div class="panel-head"><div><h2>调用日志</h2><p>每页 10 条；可按日期筛选并导出。</p></div></div>${filterBar('usage', state.usageFilters)}<div class="table-wrap"><table><thead><tr><th>时间</th><th>模型</th><th>输入</th><th>输出</th><th>总 Token</th><th>官方参考</th><th>实扣</th><th>价格版本</th><th>结果</th></tr></thead><tbody>${rows.map((u) => { const total = Number(u.input_tokens) + Number(u.output_tokens); return `<tr><td>${formatDate(u.created_at)}</td><td><code>${escapeHtml(u.model_id)}</code></td><td>${formatNumber(u.input_tokens)}</td><td>${formatNumber(u.output_tokens)}</td><td><strong>${formatNumber(total)}</strong></td><td>${formatPower(u.official_cost_micros, 6)}</td><td><strong>${formatPower(u.charged_cost_micros, 6)}</strong><span class="cell-note">${formatFactor(u.effective_billing_factor)}</span></td><td>${escapeHtml(u.pricing_label_snapshot || '-')} · V${u.pricing_version_snapshot || '-'}</td><td><span class="tag ${u.status === 'success' ? 'success' : u.status === 'blocked' ? 'warning' : 'danger'}">${{ success: '成功', blocked: '已拦截', failed: '失败' }[u.status]}</span></td></tr>`; }).join('')}</tbody></table>${rows.length ? '' : emptyState('暂无调用记录', '完成首次模型调用后会显示在这里')}</div>${pagination('usage', state.customer.usagePagination)}</section>`;
  },
  billing() {
    const data = state.customer;
...
const adminViews = {
  ...
  routes() {
    ...
    $('#pageContent').innerHTML = `<div class="content-grid"><section class="panel span-3"><div class="panel-head"><div><h2>已上架 API 服务</h2><p>价格顺序为输入 · 缓存读取 · 输出；官方参考价只做对比。</p></div></div><div class="table-wrap"><table><thead><tr><th>客户 / 模式</th><th>对外服务</th><th>协议 / 供应商模型</th><th>客户价（输入 · 缓存读取 · 输出）</th><th>官方参考</th><th>版本</th><th>状态</th><th></th></tr></thead><tbody>${data.routes.map((r) => `<tr><td>${escapeHtml(r.tenant_name)}<span class="cell-note">${r.service_mode === 'managed' ? '托管部署' : '开发者网关'}</span></td><td><strong>${escapeHtml(r.display_name)}</strong><code>${escapeHtml(r.public_model_id)}</code></td><td><strong>${protocolLabel(r.protocol)}</strong><code>${escapeHtml(r.upstream_model_id)}</code><span class="cell-note">成本 ${formatPowerPrice(r.upstream_input_power_per_million)} · ${formatPowerPrice(r.upstream_cached_input_power_per_million)} · ${formatPowerPrice(r.upstream_output_power_per_million)}</span></td><td>${formatPowerPrice(r.customer_input_power_per_million)} · ${formatPowerPrice(r.customer_cached_input_power_per_million)} · ${formatPowerPrice(r.customer_output_power_per_million)}</td><td>${formatPowerPrice(r.reference_input_power_per_million)} · ${formatPowerPrice(r.reference_cached_input_power_per_million)} · ${formatPowerPrice(r.reference_output_power_per_million)}</td><td>${escapeHtml(r.pricing_label)} · V${r.pricing_version}</td><td>${statusTag(r.active)}</td><td><button class="button text" data-action="edit-route" data-id="${r.id}">发布价格</button><button class="button text" data-action="toggle-route" data-id="${r.id}" data-active="${!r.active}">${r.active ? '下架' : '上架'}</button></td></tr>`).join('')}</tbody></table>${data.routes.length ? '' : emptyState('还没有 API 服务', '录入凭证后为客户添加服务')}</div></section>
      <section class="panel span-3"><div class="panel-head"><div><h2>供应商连接</h2><p>连接在全平台共享：一枚上游 Key 可以给多个账户使用，各账户仍独立计费。建议用“模型 · 供应商分组”命名。</p></div></div><div class="credential-grid">${data.credentials.map((c) => `<article><span>${Number(c.customer_count)} 个账户 · ${Number(c.route_count)} 个服务使用</span><strong>${escapeHtml(c.label)}</strong><code>${escapeHtml(c.base_url)}</code><small>${protocolLabel(c.protocol)} · ${c.active ? '启用' : '停用'}</small><div><button class="button secondary small" data-action="edit-credential" data-id="${c.id}">编辑</button><button class="button text small" data-action="delete-credential" data-id="${c.id}" data-routes="${c.route_count}" data-keys="${c.key_count}">删除</button></div></article>`).join('') || emptyState('还没有供应商连接', '点击“添加 API 服务”时直接粘贴供应商 Key')}</div></section></div>`;
  },
  keys() {
  ...
  pricing() {
    ...
    $('#pageContent').innerHTML = `<div class="metrics-grid">
      ...
      <section class="panel span-2"><div class="panel-head"><div><h2>结算说明</h2><p>每次调用按供应商返回的 Token 用量和当次价格版本结算。</p></div></div><div class="formula-box"><div><span>客户实扣</span><strong>以计量日志中的实扣电力为准</strong></div><div><span>官方参考</span><strong>用于价格对比展示</strong></div><div><span>价格发布</span><strong>版本 +1 并生成通知</strong></div></div></section>
  ...
  docs() {
    ...
    $('#pageContent').innerHTML = `<section class="panel docs"><div class="panel-head"><div><h2>从零开通一个 API 服务</h2><p>推荐顺序：创建使用账户 → 添加 API 服务 → 生成交付 Key → 充值。供应商 Key 和模型已经合并为一个向导。</p></div></div>
      ...
      <div class="step"><b>02</b><div><h3>一次添加供应商 Key 和模型</h3><p>进入“模型配置”点击“添加 API 服务”。选定账户后，可以复用全平台任意已有供应商连接；没有合适的 Key 时再在同一个窗口粘贴新 Key。YYLX Anthropic 地址和协议已经预填。</p><div class="guide-callout"><strong>供应商 Key 应该怎么分？</strong><span>建议在 YYLX 按“模型 / 价格分组”建 Key，在这里建立同名连接，例如“GPT-5.6 · 优惠组”。同一连接可以给多个账户使用，但每个账户的价格、余额、日志和账本独立。YYLX 分组已绑在上游 Key 上，中转站无需再填一次。</span></div><div class="guide-grid fields"><article><strong>所属账户</strong><p>决定余额、日志、模型和交付 Key 归属。</p></article><article><strong>供应商连接</strong><p>是全平台共享的上游线路，优先复用已有连接。</p></article><article><strong>供应商 API Key</strong><p>只在新增连接时粘贴，系统加密保存且不会回显。</p></article><article><strong>Base URL</strong><p>默认 <code>https://app.yylx.io/v1/messages</code>，可按需修改。</p></article><article><strong>协议</strong><p>默认 Anthropic；接 OpenAI 格式模型时再修改。</p></article><article><strong>服务模式</strong><p>自己写代码接入选开发者网关；你负责维护的项目选托管部署。</p></article></div></div></div>
      <div class="step"><b>03</b><div><h3>填写模型和价格</h3><p>同一个向导继续填写模型信息和三组价格，每个输入框下方都有用途说明。</p><div class="guide-grid fields"><article><strong>对外模型 ID</strong><p>交付并写进项目配置的稳定名称，换上游时尽量保持不变。</p></article><article><strong>供应商模型 ID</strong><p>YYLX 模型列表里的精确名称，必须完全一致。</p></article><article><strong>展示名称</strong><p>控制台里便于人阅读，例如“Dumbo Claude”。</p></article><article><strong>价格标签</strong><p>例如“优惠期”“正式价格”，会随日志和通知展示。</p></article></div>
        <h3>三组价格怎么填</h3><div class="guide-grid"><article><strong>客户成交价</strong><p>输入、缓存读取和输出分别按每 100 万 Token 填写。</p></article><article><strong>官方参考价</strong><p>仅供客户对比和计算展示倍率。</p></article><article><strong>供应商采购成本</strong><p>仅管理员可见，用于核算服务成本。</p></article><article><strong>缓存读取价</strong><p>不填时默认使用输入价，也可根据实际价格单独配置。</p></article></div>
      </div></div>
...
function credentialOptions() {
  const rows = state.admin.credentials.filter((item) => item.active);
...
function serviceFormBody() {
  ...
  return `<div class="guide-callout"><strong>最省心的用法</strong><span>建议在 YYLX 按“模型 / 价格分组”创建 Key，再在这里建立同名连接。连接可以给多个账户共享，每个账户的价格、余额、日志和账本仍完全独立。</span></div>
    ...
    <h3>2. 连接供应商</h3><label>使用哪枚供应商 Key<select id="serviceCredentialId" name="credentialId">${credentialOptions()}</select><small class="field-help">这里列出全部已有连接，可以跨账户重复使用；没有合适的再粘贴新 Key。</small></label>
    <div id="newServiceCredentialFields"><div class="form-grid"><label>连接名称<input id="serviceCredentialLabel" name="credentialLabel" value="YYLX · 模型/分组" /><small class="field-help">建议写“GPT-5.6 · 优惠组”或“Claude · 默认组”，便于和 YYLX Key 一一对应。</small></label><label>协议<select name="credentialProtocol"><option value="anthropic" selected>Anthropic</option><option value="openai">OpenAI</option></select><small class="field-help">YYLX Claude 默认使用 Anthropic；需要接 OpenAI 模型时再改。</small></label></div><label>供应商 Base URL<input name="credentialBaseUrl" value="https://app.yylx.io/v1/messages" /><small class="field-help">已预填 YYLX Anthropic 地址，供应商地址变化时才需要修改。</small></label><label>供应商 API Key<input id="serviceApiKey" name="apiKey" type="password" autocomplete="off" /><small class="field-help">粘贴与上面名称对应的 YYLX Key；系统加密保存且不会回显。</small></label></div>
    ...
    <h3>4. 设置价格（电力 / 100 万 Token）</h3><p class="form-hint">成交价决定实际扣费；官方参考价用于展示对比；采购成本仅管理员可见。</p><h3>成交价</h3><div class="form-grid three"><label>输入<input name="customerInputPrice" type="number" min="0" step="0.000001" required /></label><label>缓存读取（可选）<input name="customerCachedInputPrice" type="number" min="0" step="0.000001" /></label><label>输出<input name="customerOutputPrice" type="number" min="0" step="0.000001" required /></label></div><h3>官方参考价</h3><div class="form-grid three"><label>输入<input name="referenceInputPrice" type="number" min="0" step="0.000001" required /></label><label>缓存读取（可选）<input name="referenceCachedInputPrice" type="number" min="0" step="0.000001" /></label><label>输出<input name="referenceOutputPrice" type="number" min="0" step="0.000001" required /></label></div><h3>供应商采购成本（仅管理员可见）</h3><div class="form-grid three"><label>输入<input name="upstreamInputPrice" type="number" min="0" step="0.000001" value="0" required /></label><label>缓存读取（可选）<input name="upstreamCachedInputPrice" type="number" min="0" step="0.000001" /></label><label>输出<input name="upstreamOutputPrice" type="number" min="0" step="0.000001" value="0" required /></label></div>`;
}

function syncServiceCredentialFields() {
  const select = $('#serviceCredentialId');
  const existingRows = state.admin.credentials.filter((item) => item.active);
  select.innerHTML = credentialOptions();
...
async function handleAction(button) {
  ...
  if (action === 'delete-credential') {
    const routeCount = Number(button.dataset.routes || 0);
    const keyCount = Number(button.dataset.keys || 0);
    const warning = routeCount
      ? `这枚连接正在被 ${routeCount} 个 API 服务使用。\n\n继续删除会同时删除这些服务${keyCount ? `，并使 ${keyCount} 枚已绑定 Key 无法继续调用` : ''}。历史日志和账本会保留。\n\n确定继续吗？`
      : '确定删除这枚未使用的供应商连接吗？删除后无法恢复。';
